...
namespace WatchStore.Areas.Admin.Controllers
{
    public class SliderController : BaseController
    {
        ...
        {
            ViewBag.count_trash = db.Sliders.Where(m => m.Status == 0).Count();
            ViewBag.Orders = new SelectList(db.Sliders.Where(m => m.Status != 0).ToList(), "Id", "Position", 0);
            return View();
        }
        ...
        public ActionResult Create(pSlider modelSliders)
        {

            if (ModelState.IsValid)
            {
                var modelSlider = new MSlider();
                String strSlug = MyString.ToAscii(modelSliders.Name);
                String slug = strSlug;
                modelSlider.Link = slug;
                modelSlider.Position = modelSliders.Position;
                ...
                ViewBag.Orders = new SelectList(db.Sliders.Where(m => m.Status != 0).ToList(), "Id", "Position", 1);
