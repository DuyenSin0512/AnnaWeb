...
namespace WatchStore.Areas.Admin.Controllers
{
    public class ProductController : BaseController
    {
        ...
        // GET: Admin/Product
        public ActionResult Index(string searchAdmin = null)
        { 
            ...
            {
                data = list.Where(x=>x.ProductName.Contains(searchAdmin)).ToList();
            }
            return View(data);
        }
        ...
        public ActionResult Edit(MProduct mProduct)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (file != null && file.Count > 0)
                {
                    ...
                    mProduct.Image = filename;
                    String Strpath = Path.Combine(Server.MapPath("~/Public/library/product/"), filename);
                    currentFile.SaveAs(Strpath);
                    if (file.Count > 1)
                    {
                        HttpPostedFileBase currentFile1 = Request.Files[1];
                        String filename1 = currentFile1.FileName;
                        mProduct.img1 = filename1;
                        String Strpath1 = Path.Combine(Server.MapPath("~/Public/library/product/"), filename1);
                        currentFile1.SaveAs(Strpath1);
                    }
                    if (file.Count > 2)
                    {
                        HttpPostedFileBase currentFile2 = Request.Files[2];
                        String filename2 =  currentFile2.FileName;
                        mProduct.img2 = filename2;
                        String Strpath2 = Path.Combine(Server.MapPath("~/Public/library/product/"), filename2);
                        currentFile2.SaveAs(Strpath2);
                    }
                    if (file.Count > 3)
                    {
                        HttpPostedFileBase currentFile3 = Request.Files[3];
                        String filename3 = currentFile3.FileName;
                        mProduct.img3 = filename3;
                        String Strpath3 = Path.Combine(Server.MapPath("~/Public/library/product/"), filename3);
                        currentFile3.SaveAs(Strpath3);
                    }
                    if (file.Count > 4)
                    {
                        HttpPostedFileBase currentFile4 = Request.Files[4];
                        String filename4 = currentFile4.FileName;
                        mProduct.img4 = filename4;
                        String Strpath4 = Path.Combine(Server.MapPath("~/Public/library/product/"), filename4);
                        currentFile4.SaveAs(Strpath4);
                    }
        ...
        {
            MProduct mProduct = db.Products.Find(id);
            mProduct.Discount = (mProduct.Discount == 2) ? 3 : 2;
