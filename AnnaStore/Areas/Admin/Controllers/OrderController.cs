...
namespace WatchStore.Areas.Admin.Controllers
{
    public class OrderController : BaseController
    {
        ...
        {
            MOrder mOrder = db.Orders.Find(id);
            if (op == 1) { mOrder.Status = 1; } else if (op == 2) { mOrder.Status = 2; } else  if(op == 3) { mOrder.Status = 3; } else { mOrder.Status = 0; }
            var adminId = Session["Admin_ID"];
            
            mOrder.Updated_at = DateTime.Now;
            mOrder.Updated_by = adminId == null? -1 : int.Parse(adminId.ToString());
            db.Entry(mOrder).State = EntityState.Modified;
            db.SaveChanges();
            if (mOrder.Status == 0)
            {
                Notification.set_flash("Đã huỷ thành công!", "success");
            }
            else
                Notification.set_flash("Cập nhật thành công!", "success");
            
            return Json(new { s = mOrder.Status});
        }
